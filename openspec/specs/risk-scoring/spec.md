...
Turn reported crimes into deterministic, versioned, evaluable per-edge risk scores over the CABA
walkable graph using **Network Temporal KDE** (`network_temporal_edge_risk_v1`): crime influence
propagated by walking-network distance and weighted by severity, weapon, motorcycle, recency, time
bucket, and weekday type. All computation runs offline in `etl/risk_network_kde/`; the Go API only
reads the resulting tables (`edge_risk_scores`, `edge_risk_score_components`,
`risk_model_versions`). No machine learning in this capability.
