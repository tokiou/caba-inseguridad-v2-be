...
type Repository interface {
	...
	// RouteRiskByBucket returns the per-edge risk_score for the given edges
	// across the four time buckets and a fixed weekday type, for one model. It
	// feeds the per-route time-of-day risk profile; edges without a row for a
	// bucket simply have no entry (treated as zero risk by the caller).
	RouteRiskByBucket(ctx context.Context, edgeIDs []int64, modelVersionID int64, weekdayType string) ([]EdgeBucketRisk, error)
}
...
SELECT
    ...
    COALESCE(json_agg(json_build_object(
        ...
        'point_lng', ST_X(ST_LineInterpolatePoint(geom, 0.5)),
        'point_lat', ST_Y(ST_LineInterpolatePoint(geom, 0.5))
    ) ORDER BY seq), '[]')::text
FROM enriched`
...
func (r *PostgresRepository) RouteRiskByBucket(
	ctx context.Context, edgeIDs []int64, modelVersionID int64, weekdayType string,
) ([]EdgeBucketRisk, error) {
	rows, err := r.pool.Query(ctx, routeRiskByBucketQuery, edgeIDs, modelVersionID, weekdayType)
	if err != nil {
		return nil, fmt.Errorf("saferoutes: route risk by bucket: %w", err)
	}
	defer rows.Close()

	var out []EdgeBucketRisk
	for rows.Next() {
		var e EdgeBucketRisk
		if err := rows.Scan(&e.EdgeID, &e.TimeBucket, &e.RiskScore); err != nil {
			return nil, fmt.Errorf("saferoutes: scan bucket risk: %w", err)
		}
		out = append(out, e)
	}
	if err := rows.Err(); err != nil {
